...
!function (a) {
    ...
    a.extend(a.FE.DEFAULTS, {
        lineHeight: ["inherit", "10px", "11px", "12px", "13px", "14px", "15px", "16px", "17px", "18px", "19px", "20px", "22px", "24px", "26px", "28px","30px","40px","50px","60px","70px","80px","90px","100px"],
        ...
        type: "dropdown", title: "行间距", displaySelection: function (a) {
...
!function (a) {
    ...
    a.extend(a.FE.DEFAULTS, {
        letterSpacing: ["inherit", "1px", "2px", "3px", "4px", "5px", "6px", "7px", "8px", "9px", "10px", "20px", "30px", "40px", "50px", "60px"],
        letterSpacingSelection: !1,
        letterSpacingDefaultSelection: "1.4"
    }), a.FE.PLUGINS.letterSpacing = function (b) {
        function c(c) {
            b.selection.save(), b.html.wrap(!0, !0, !0, !0), b.selection.restore();
            for (var d = b.selection.blocks(), e = 0; e < d.length; e++) {
                a(d[e]).css("letter-spacing", c).removeClass("fr-temp-div");
            }
            b.selection.save(), b.html.unwrap(), b.selection.restore()
        }

        function d(c, d) {
            var e = a(b.selection.element()).css("letter-spacing");
            d.find(".fr-command.fr-active").removeClass("fr-active").attr("aria-selected", !1), d.find('.fr-command[data-param1="' + e + '"]').addClass("fr-active").attr("aria-selected", !0);
            var f = d.find(".fr-dropdown-list"), g = d.find(".fr-active").parent();
            g.length ? f.parent().scrollTop(g.offset().top - f.offset().top - (f.parent().outerHeight() / 2 - g.outerHeight() / 2)) : f.parent().scrollTop(0)
        }
        ...
        function e(c) {
            if (b.opts.letterSpacingSelection) {
                var d = a(b.selection.element()).css("letter-spacing");
                c.find("> span").text(d)
            }
        }

        return {apply: c, refreshOnShow: d, refresh: e}
    }, a.FE.RegisterCommand("letterSpacing", {
        type: "dropdown", title: "字间距", displaySelection: function (a) {
            return a.opts.letterSpacingSelection
        }, displaySelectionWidth: 30, defaultSelection: function (a) {
            return a.opts.letterSpacingDefaultSelection
        }, html: function () {
            for (var a = '<ul class="fr-dropdown-list" role="presentation">', b = this.opts.letterSpacing, c = 0; c < b.length; c++) {
                var d = b[c];
                a += '<li role="presentation"><a class="fr-command" tabIndex="-1" role="option" data-cmd="letterSpacing" data-param1="' + d + '" title="' + d + '">' + d + "</a></li>"
            }
            return a += "</ul>"
        }, callback: function (a, b) {
            this.letterSpacing.apply(b)
        }, refresh: function (a) {
            this.letterSpacing.refresh(a)
        }, refreshOnShow: function (a, b) {
            this.letterSpacing.refreshOnShow(a, b)
        }, plugin: "letterSpacing"
    }), a.FE.DefineIcon("letterSpacing", {NAME: "arrows-h"})
});
/*中文插件*/
(function (factory) {
...
$.FE.LANGUAGE['zh_cn'] = {
  translation: {
    ...
    "Keep": "保持",
    "Clean": "清洁",
    "Word Paste Detected": "检测到Office字符串"
  },
  direction: "ltr"
};
...
var froala_fontFamily= {"宋体":"宋体","微软雅黑":"微软雅黑","微軟正黑體":"微軟正黑體","黑体":"黑体","楷体":"楷体","幼圆":"幼圆","华文仿宋":"华文仿宋","arial":"arial","comic sans ms":"comic sans ms","courier new":"courier new","georgia":"georgia","lucida sans unicode":"lucida sans unicode","tahoma":"tahoma","times new roman":"times new roman","trebuchet ms":"trebuchet ms","verdana":"verdana","century gothic":"century gothic","sans-serif":"sans-serif"};

if(typeof(top.qfy_language_setting) !="undefined" && top.qfy_language_setting=="2"){
	froala_fontFamily["楷體"] = "楷體";
	froala_fontFamily["標楷體"] = "標楷體";
	froala_fontFamily["明體"] = "明體";
}
if(typeof(top.google_names) !="undefined" && top.google_names!=""){
	var tmp_froala_fontFamily=top.google_names.split(';');
	for(var i=0;i<tmp_froala_fontFamily.length;i++)
	{
		froala_fontFamily[tmp_froala_fontFamily[i]] =tmp_froala_fontFamily[i];
	}
}
function textarea_editor_new(obj,height,width){
	  if(!width) width="auto";
	  if(!height) height=120;
		...
		obj.froalaEditor({
			...
			toolbarButtons: [  'fontFamily', 'fontSize', 'color', 'paragraphFormat','bold', 'italic', 'underline', 'strikeThrough','lineHeight','letterSpacing','|', 'align', 'formatOL', 'formatUL', 'outdent', 'indent',  'insertLink2', 'insertImage2','insertBtn','insertTable','specialCharacters','|', 'selectAll', 'clearFormatting','html', 'undo', 'redo'],
		...
		obj.parent().find(".fr-box table").addClass("qtable");
		
}
function fr_link_event(obj){
	//fr-box fr-ltr fr-basic fr-top
	var editorbox = jQuery('.fr-box .fr-image-resizer.fr-active:visible').closest(".fr-box");
	if(editorbox.hasClass("fr-top")){
		var editor = editorbox.next("textarea:first").data('froala.editor');
	}else{
		var editor = editorbox.data('froala.editor');
	}

	editor.commands.exec('linkMore');
}
jQuery(function($){
	$.FroalaEditor.RegisterCommand('保存', {
	  title: false,
	  focus: false,
	  undo: false,
	...
	$.FroalaEditor.RegisterCommand('取消', {
	  title: false,
	  focus: false,
	  undo: true,
	  refreshAfterCallback: false,
	});
	$.FroalaEditor.DefineIcon('linkMore', {NAME: 'search'});
	$.FroalaEditor.RegisterCommand('linkMore', {
		  title: "链接",
		  focus: false,
		  undo: true,
		  refreshAfterCallback: false,
		  callback: function (cmd, val) {
			  var src = '/FeiEditor/bitSite/wp/bottstarpLink.php?noseo=1';
			  var img = this.image.get();
			  var link = this.link;
			  var $el = this.$box;
			  if(link){
				    var url = $(".fr-popup [name='href']").val();
				    var target = $(".fr-popup [name='target']:checked").length>0?"target=_blank":"";
				    var linkhtml = $("<a href='"+url+"'   "+target+"></a>").prop("outerHTML");
				    if(top.iframeVCWin){
						var src='/FeiEditor/bitSite/wp/bottstarpLink.php?noseo=1&tag='+new Date().getTime()+'&element='+encodeURIComponent(top.iframeVCWin.base64_encode(linkhtml));
					}else{
						var src='/FeiEditor/bitSite/wp/bottstarpLink.php?noseo=1&tag='+new Date().getTime()+'&element='+encodeURIComponent(parent.base64_encode(linkhtml));
					}
			  }
			
			  top.qfy_popinfo_fun("<div class='dialog_contents' style='box-shadow:0 0 3px rgba(0, 0, 0, .15);border-top:5px solid #222;padding:20px;background:#fff;width:600px;height:440px;'><iframe src='"+src+"'  style='width:600px;height:400px;border: 0;' /><div style='    text-align: right;'><input type='button' class=' btn_insert_link' value='应用' >&nbsp;&nbsp;<input type='button' class='btn_close_link' value='关闭' ></div></div>");
			  top.jQuery(".qfy_popinfo").css("z-index","13336").css("margin-top","-220px").width("600");
              jQuery(".fr-popup.fr-active").removeClass("fr-active");
              top.jQuery(".qfy_popinfo .btn_insert_link").unbind().bind("click",function(e){

				  if($el.find('.fr-popup.fr-active').length==0){
						img.click();
						link.imageLink();
					}
					var iconFrame = top.jQuery('.dialog_contents iframe')[0].contentWindow;
					var obj=iconFrame.jQuery('#result a');
					iconFrame.jQuery("#btn-seo").closest(".form-group").remove();
					var url=obj.attr("href");
					if(url==""||url=="undefined"){
						url = "";
					}
					var target=obj.attr("target")?obj.attr("target"):"";
					 $(".fr-popup [name='href']").val(url);
					 if(target=="_blank"){
						 $(".fr-popup [name='target']").attr("checked","checked");
					 }else{
						 $(".fr-popup [name='target']").removeAttr("checked");
					 }
                  top.jQuery(".qfy_popinfo").remove();
					
					return;
			  })
              top.jQuery(".qfy_popinfo .btn_close_link").unbind().bind("click",function(){
                  top.jQuery(".qfy_popinfo").remove();
				   if($el.find('.fr-image-resizer.fr-active').length==0){
						img.click();
						link.imageLink();
					}
			  })
	 ...
	 $.FroalaEditor.DefineIcon('tableBorderSize', {NAME: 'text-height'});
	$.FroalaEditor.RegisterCommand('tableBorderSize', {
		  title: "设置表格边框大小",
	      type: 'dropdown',
	      focus: false,
	      undo: false,
	      ...
	      callback: function (cmd, val) {
	    	 ...
	    	 table.find("td,th").css("border-width",val);
	      },
	      // Callback on refresh.
	      ...
	      refreshOnShow: function ($btn, $dropdown) {
	    	  var table = this.$el.find(".fr-selected-cell").closest("table");
			  var val = table.find("td:first").css("border-width");
			  $dropdown.find(".fr-command.fr-active").removeClass("fr-active").attr("aria-selected", !1);
			  $dropdown.find('.fr-command[data-param1="' + val + '"]').addClass("fr-active").attr("aria-selected", !0);
 
	      }
		});
	 $.FroalaEditor.DefineIcon('tableBorderColor', {NAME: 'square-o'});
	$.FroalaEditor.RegisterCommand('tableBorderColor', {
		  title: '设置表格边框颜色',
		  focus: false,
		  undo: true,
		  callback: function (cmd, val) {
			  var table = this.$el.find(".fr-selected-cell").closest("table");
			  var curr = this.$el.find(".fr-selected-cell");
			  this.table.showColorsPopup();
		      $(".fr-popup .fr-table-colors .fr-command,.fr-popup .fr-table-colors-hex-layer .fr-submit").unbind();
			 $(".fr-popup:visible .fr-table-colors-hex-layer input").val("");
			 var bgcolor = curr.css("background-color");
			 $(".fr-popup:visible .fr-table-colors .fr-command").bind('click',function(e){
					e.stopPropagation();
					e.preventDefault();
					var val = $(this).attr("data-param1");
			    	table.find("td,th").css("border-color",val);
			    	//$(".fr-popup .fr-table-colors-hex-layer input").val("");
			    	if(bgcolor!=val){
			    		curr.css("background-color",bgcolor);
			    		setTimeout(function(){curr.css("background-color",bgcolor);},50);
			    	}
			    	return true;
			 })
			 $(".fr-popup:visible .fr-table-colors-hex-layer .fr-submit").bind('click',function(e){
					e.stopPropagation();
					e.preventDefault();
			    	var val = $(this).closest(".fr-table-colors-hex-layer").find("input").val();

			    	if(val) table.find("td,th").css("border-color",val);
			    	else  table.find("td,th").css("border-color","transparent");
			    	if(bgcolor!=val){
			    		curr.css("background-color",bgcolor);
			    		setTimeout(function(){curr.css("background-color",bgcolor);},50);
			    	}
			 ...
			 $(".fr-popup:visible .fr-table-colors-hex-layer input").bind('keydown',function(e){
				 if (e.keyCode == 13){
					e.stopPropagation();
					e.preventDefault();
			    	var val = $(this).val();
	 ...
	 $.FroalaEditor.DefineIcon('tableBorderpadding', {NAME: 'arrows-h'});
	$.FroalaEditor.RegisterCommand('tableBorderpadding', {
		  title: "设置表格单元格左右内间距",
	      type: 'dropdown',
	      focus: false,
	      undo: false,
	      ...
	      options: {
	        ...
	        '11px': '11px','12px': '12px','13px': '13px','14px': '14px','15px': '15px','16px': '16px','17px': '17px','18px': '18px','19px': '19px','20px': '20px',
	        '30px': '30px',
	        '40px': '40px',
	        '50px': '50px',
	      },
	      callback: function (cmd, val) {
	    	 var table = this.$el.find(".fr-selected-cell").closest("table");
	    	 table.find("td,th").css("padding-left",val).css("padding-right",val);
	      },
	      // Callback on refresh.
	      refresh: function ($btn) {
	      ...
	      refreshOnShow: function ($btn, $dropdown) {
	    	  var table = this.$el.find(".fr-selected-cell").closest("table");
			  var val = table.find("td:first").css("padding-left");
			  $dropdown.find(".fr-command.fr-active").removeClass("fr-active").attr("aria-selected", !1);
			  $dropdown.find('.fr-command[data-param1="' + val + '"]').addClass("fr-active").attr("aria-selected", !0);
 
	      }
		});
	 $.FroalaEditor.DefineIcon('tableBorderpaddingv', {NAME: 'arrows-v'});
		$.FroalaEditor.RegisterCommand('tableBorderpaddingv', {
			  title: "设置表格单元格上下内间距",
		      type: 'dropdown',
		      focus: false,
		      undo: false,
		      ...
		      callback: function (cmd, val) {
		    	 var table = this.$el.find(".fr-selected-cell").closest("table");
		    	 table.find("td,th").css("padding-top",val).css("padding-bottom",val);
		      },
		      // Callback on refresh.
		      refresh: function ($btn) {
		      ...
		      refreshOnShow: function ($btn, $dropdown) {
		    	  var table = this.$el.find(".fr-selected-cell").closest("table");
				  var val = table.find("td:first").css("padding-top");
				  $dropdown.find(".fr-command.fr-active").removeClass("fr-active").attr("aria-selected", !1);
				  $dropdown.find('.fr-command[data-param1="' + val + '"]').addClass("fr-active").attr("aria-selected", !0);
	 ...
	 $.FroalaEditor.DefineIcon('insertLink2', {NAME: 'link'});
	$.FroalaEditor.RegisterCommand('insertLink2', {
		  title: "链接",
		  focus: false,
		  undo: true,
		  refreshAfterCallback: false,
		  callback: function () {
			  ...
			  if($this.selection.text()!="" ||  link  ){
				  var src = '/FeiEditor/bitSite/wp/bottstarpLink.php';
				  if(link){
					    var linkhtml = $(link).prop("outerHTML");
					    if(top.iframeVCWin){
							var src='/FeiEditor/bitSite/wp/bottstarpLink.php?tag='+new Date().getTime()+'&element='+encodeURIComponent(top.iframeVCWin.base64_encode(linkhtml));
						}else{
							var src='/FeiEditor/bitSite/wp/bottstarpLink.php?tag='+new Date().getTime()+'&element='+encodeURIComponent(parent.base64_encode(linkhtml));
						}
				  }
				  $this.selection.save();
				  top.qfy_popinfo_fun("<div class='dialog_contents' style='box-shadow:0 0 3px rgba(0, 0, 0, .15);border-top:5px solid #222;padding:20px;background:#fff;width:600px;height:440px;'><iframe src='"+src+"'  style='width:600px;height:400px;border: 0;' /><div style='    text-align: right;'><input type='button' class=' btn_insert_link' value='应用' >&nbsp;&nbsp;<input type='button' class='btn_close_link' value='关闭' ></div></div>");
				  top.jQuery(".qfy_popinfo").css("z-index","133636").css("margin-top","-220px").width("600");
				  top.jQuery(".qfy_popinfo .btn_insert_link").unbind().bind("click",function(){
					   var iconFrame = top.jQuery('.dialog_contents iframe')[0].contentWindow;
						var obj=iconFrame.jQuery('#result a');
						var url=obj.attr("href");
						var flag = iconFrame.jQuery('#pageUrlType').val();
						if(url==""||url=="undefined"){
							jAlert("请填写url");
							return false;
						}
						...
						if(target=="_blank"){
							if($(link).hasClass("bitButton")){
								$(link).attr("target",target).attr("rel",rel).attr("href",url);
							}else{
								$this.link.insert(url,"",{'target':target, 'rel': rel});
							}
							$(link).attr("url-flagtarget","_blank");
						}else{
							if($(link).hasClass("bitButton")){
								$(link).removeAttr("target").attr("rel",rel).attr("href",url);
							}else{
								$this.link.insert(url,"",{ 'rel': rel});
							}
							$(link).attr("url-flagtarget","");
						}
						top.jQuery(".qfy_popinfo").remove();
				  ...
				  top.qfy_popinfo_fun("<div style='padding:20px;'>请先选择一段区域来添加链接！</div>",2);
			  }
	...
	$.FroalaEditor.RegisterCommand('insertImage2', {
		  ...
		  callback: function () {
			  var $this = this;
			  var pfun = typeof revOpenAddImageDialog =="function"?revOpenAddImageDialog:"";
			  var imgfun = parent.revOpenAddImageDialog?parent.revOpenAddImageDialog:pfun;
			  imgfun = typeof imgfun =="function"?imgfun:bitOpenNewImageDialog;
			  imgfun('替换图片', function(urlImage, imageID,alt,title) {
					if(urlImage){
						if(!title) title ="";
						if(!alt) alt ="";
						$this.html.insert("<img src='"+urlImage+"' alt='"+alt+"' title='"+title+"'  />");
					}
			},false);
		  }
		});
	 $.FroalaEditor.DefineIcon('imageReplace2', {NAME: 'exchange'});
		$.FroalaEditor.RegisterCommand('imageReplace2', {
			  title: "替换图片",
			  focus: false,
			  undo: true,
			  refreshAfterCallback: false,
	...
	$.FroalaEditor.DefineIcon('insertBtn', {NAME: 'pause'});
	$.FroalaEditor.RegisterCommand('insertBtn', {
		  title: "插入按钮",
		  focus: false,
		  undo: true,
		  refreshAfterCallback: false,
		  callback: function () {
				 ...
				 var src = '/FeiEditor/bitSite/wp/bottstarpButton.php';
				 
				  top.qfy_popinfo_fun("<div class='dialog_contents' style='box-shadow:0 0 3px rgba(0, 0, 0, .15);border-top:5px solid #222;padding:20px;background:#fff;width:880px;height:560px;'><iframe src='"+src+"'  style='width:880px;height:520px;border: 0;' /><div style='    text-align: right;'><input type='button' class=' btn_insert_link' value='应用' >&nbsp;&nbsp;<input type='button' class='btn_close_link' value='关闭' ></div></div>");
				  top.jQuery(".qfy_popinfo").css("z-index","13389").css("margin-top","-220px").width("880");
                  top.jQuery(".qfy_popinfo .btn_insert_link").unbind().bind("click",function(){
					    var iFrame = top.jQuery('.dialog_contents iframe')[0].contentWindow;
						var obj=iFrame.jQuery('#button a');
						$this.html.insert($(obj).prop("outerHTML"));
                        top.jQuery(".qfy_popinfo").remove();
	...
	$.FroalaEditor.DefineIcon('insertBtn2', {NAME: 'edit'});
	$.FroalaEditor.RegisterCommand('insertBtn2', {
		  title: "编辑",
		  focus: false,
		  undo: true,
		  refreshAfterCallback: false,
				 ...
				 if($(link).hasClass("bitButton")){
						 var src = '/FeiEditor/bitSite/wp/bottstarpButton.php';
						  if(link){
								var linkhtml = $(link).prop("outerHTML");
								if(top.iframeVCWin){
									var src='/FeiEditor/bitSite/wp/bottstarpButton.php?tag='+new Date().getTime()+'&element='+encodeURIComponent(top.iframeVCWin.base64_encode(linkhtml));
								}else{
									var src='/FeiEditor/bitSite/wp/bottstarpButton.php?tag='+new Date().getTime()+'&element='+encodeURIComponent(parent.base64_encode(linkhtml));
								}
						  }
						  top.qfy_popinfo_fun("<div class='dialog_contents' style='box-shadow:0 0 3px rgba(0, 0, 0, .15);border-top:5px solid #222;padding:20px;background:#fff;width:880px;height:560px;'><iframe src='"+src+"'  style='width:880px;height:520px;border: 0;' /><div style='    text-align: right;'><input type='button' class=' btn_insert_link' value='应用' >&nbsp;&nbsp;<input type='button' class='btn_close_link' value='关闭' ></div></div>");
						  top.jQuery(".qfy_popinfo").css("z-index","1336").css("margin-top","-220px").width("880");
                          top.jQuery(".qfy_popinfo .btn_insert_link").unbind().bind("click",function(){
								var iFrame = top.jQuery('.dialog_contents iframe')[0].contentWindow;
								var obj=iFrame.jQuery('#button a');
					
								$(link).replaceWith($(obj).prop("outerHTML"));
                          top.jQuery(".qfy_popinfo").remove();
	...
	$.FroalaEditor.DefineIcon('linkRemove2', {NAME: 'remove'});
	$.FroalaEditor.RegisterCommand('linkRemove2', {
		  title: "删除",
		  focus: false,
		  undo: true,
		  refreshAfterCallback: false,
